...
bot.dialog('/', [
    function (session) {
        ...
        var card = new builder.HeroCard(session)
            .title("IBot")
            .text("Your bot - to answer insurance queries")
            .images([
                 builder.CardImage.create(session, "http://docs.botframework.com/images/demo_bot_image.png")
            ]);
        var msg = new builder.Message(session).attachments([card]);
        session.send(msg);
        session.send("Hi... I'm the Bot to help you understand car insurance policies");
...
bot.dialog('/auto', [
  function (session) {
        ...
        builder.Prompts.text(session, "Which state?");
    },
    function (session,results) {
        var state = results.response;
        session.send("You chose '%s'", state );
        session.send("Glad to help you with auto insurance queries. Just follow the prompts and you can quit at any time by saying 'cancel'.");
        builder.Prompts.choice(session, "Select an area you are interested in", "Minimum Car Insurance Coverage|Teen Driving Laws|Good Driver Discount");
    },
    function (session, results) {
        session.send("You chose '%s'", results.response.entity);
        session.send("Bodily Injury Liability Coverage: $15,000 per person / $30,000 per accident minimum ");
